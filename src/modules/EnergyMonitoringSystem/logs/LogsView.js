...
class LogsScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
        ...
        sensorarray: [],
        selected1: 0,
        selectedValue1 : this.props.sensor[0].name,
        selected2: 0,
        selectedValue2 : 'Va',
  ...
  async componentDidMount () {
    ...
    const{selectedValue2,selected1,startDate,endDate,selectedValue1} = this.state;
    let done = null;
    ...
    done = await this.props.get_Em_sensor(user.id);

    if (done === 'done'){
      this.handleSensorData();  
    }

    const Index2 = sensor.findIndex(s=> s.name === selectedValue1);
    await this.props.getEmLogs(selectedValue2,sensor[Index2]._id,startDate,endDate);
  ...
  handleSensorChange = (index,value) => {
    ...
    const Index2 = sensor.findIndex(s=> s.name === value);

    this.setState({
      selected1: index,
      selectedValue1: sensor[Index2].name
    });

    if(selectedValue2 !== null){
      this.props.getEmLogs(selectedValue2,sensor[Index2]._id,startDate,endDate);
    }
  }
  ...
  handleTypeChange = (index,value) => {
   ...
   this.props.getEmLogs(value,sensor[selected1]._id,startDate,endDate);
  }
  ...
  startDateChange = (date) => {
    ...
    this.props.getEmLogs(selectedValue2,sensor[selected1]._id,date,endDate);
  ...
  endDateChange = (date) => {
    ...
    this.props.getEmLogs(selectedValue2,sensor[selected1]._id,startDate,date);
  }

  render() {
    const{emData,logs,logsLoading} = this.props;
              ...
              <View >
                {logs.length > 0 ? (
                  ...
                  <FlatList                
                    ...
                    messagedata={item.logs} title= {this.state.selectedValue2}/>}
