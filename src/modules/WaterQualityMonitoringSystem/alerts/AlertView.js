...
class AlertScreen extends React.Component {
  ...
  componentDidMount () {
    ...
    this.focusListener = navigation.addListener("didFocus", () => {
      this.props.getLmsAlerts(user.id,'qa');
  ...
  render() {
    const {waterQualityAlertData,waterQualityAlertLoading} = this.props; 
    
    return (
      <View style={styles.container}>
        
            {waterQualityAlertLoading ? (
              <View style={{flex:1, alignItems:'center',justifyContent:'center'}}>
                <Bars size={15} color={colors.primary} />
              </View>
            ) : (
              <View style= {{flex:1}}>
                {waterQualityAlertData.length > 0 ? (
                  <FlatList                
                    data={ waterQualityAlertData }                               
