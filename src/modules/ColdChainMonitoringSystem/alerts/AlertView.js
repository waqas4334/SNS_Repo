...
class AlertScreen extends React.Component {
  ...
  componentDidMount () {
    ...
    this.focusListener = navigation.addListener("didFocus", () => {
      this.props.getColdChainAlerts(user.id,'chain');
    });
  }
  ...
  render() {
    const {coldchainAlertData,coldchainAlertLoading} =this.props;    
    return (
      <View style={styles.container}>
        
            {coldchainAlertLoading ? (
              <View style={{flex:1, alignItems:'center',justifyContent:'center'}}>
                <Bars size={15} color={colors.primary} />
              </View>
            ) : (
              <View style={{flex:1}}>
                {coldchainAlertData.length > 0 ? (
                  <FlatList                
                    data={ coldchainAlertData }                               
