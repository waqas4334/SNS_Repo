...
import {  getSensors,getCharts } from '../../../redux/actions/gasAction';

const ChartsScreen = () => {
    const user = useSelector(state => state.auth.user);
    const gasModule = useSelector(state => state.gas.gasModule);
    const charts = useSelector(state => state.gas.charts);
    const chartsLoading = useSelector(state => state.gas.chartsLoading);
    const highest = useSelector(state => state.gas.highest);
    ...
    const [type, setType] = useState(['Gas Status Charts'])
    ...
    const handleSensorData = () => {
        ...
        setModuleArray(temp),
        setSelectedModule(0),
        setSelectedModuleValue (gasModule[0].name)
    }
   const  handleSensorChange = (index, value) => {
    ...
    const  getChartData = (moduleIndex,type,range) => {
        let range2 = 'week';
        let type2 = 'Gas Status Charts';

        if (type === 'Gas Status Charts') {
            type2 = 'gas_status'
        }
        ...
        if (gasModule[moduleIndex] !== undefined) {
            dispatch(getCharts(gasModule[moduleIndex]._id,type2,range2));
        }
    };
      ...
      <ScrollView style={styles.container}>
        ...
        {selectedTypeValue === null ? (
          ...
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            ...
            {selectedTypeValue === 'Gas Status Charts' ?
                         (
            <VictoryChart
                      width={width}
                      height={hp('45%')}
